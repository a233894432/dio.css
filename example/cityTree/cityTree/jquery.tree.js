...
(function($) {
  ...
  $.fn.treeview = function(settings) {
    var dfop = {
      ...
      /**
             ...
             */
      cbiconpath: "/html/assets/js/cityTree/img/",
    ...
    function buildnode(nd, ht, deep, path, isend) {
      ...
      if (nd.hasChildren) {
        cs.push(
          nd.isexpand ? "bbit-tree-node-expanded" : "bbit-tree-node-collapsed"
        );
      } else {
      ...
      if (deep == 1) {
        ht.push(
          "<img class='bbit-tree-icon' src='" + dfop.cbiconpath + "s.gif'/>"
        );
      } else if (deep > 1) {
        ht.push(
          "<img class='bbit-tree-icon' src='" + dfop.cbiconpath + "s.gif'/>"
        ...
        for (var j = 1; j < deep; j++) {
          ht.push(
            "<img class='bbit-tree-elbow-line' src='" +
              dfop.cbiconpath +
              "/s.gif'/>"
          );
        }
      ...
      if (nd.hasChildren) {
        if (nd.isexpand) {
          cs.push(
            isend ? "bbit-tree-elbow-end-minus" : "bbit-tree-elbow-minus"
          );
        } else {
    ...
    function check(item, state, type) {
      ...
      if (type == 1) {
        ...
        for (var i = 0; i < l; i++) {
          if (
            (state == 1 && cs[i].checkstate != 1) ||
            (state == 0 && cs[i].checkstate != 0)
          ) {
            ch = false;
            break;
          }
    ...
    function nodeclick(e) {
      ...
      if (et.tagName == "IMG") {
        ...
        if (
          $(et).hasClass("bbit-tree-elbow-plus") ||
          ...
          $(et).hasClass("bbit-tree-elbow-minus") ||
    ...
    function asnyloadc(pnode, isAsync, callback) {
      if (dfop.url) {
        if (pnode && pnode != null) var param = builparam(pnode);
        $.ajax({
          type: dfop.method,
          url: dfop.url,
          data: param,
          ...
          if (!$(this).hasClass("bbit-tree-elbow")) {
            $(this).hover(
              function() {
                $(this)
                  .parent()
                  .addClass("bbit-tree-ec-over");
              },
              function() {
                $(this)
                  .parent()
                  .removeClass("bbit-tree-ec-over");
              }
            );
          }
        });
    ...
    function reflash(itemId) {
      ...
      if (node.length > 0) {
        ...
        var isend =
          node.hasClass("bbit-tree-elbow-end") ||
    ...
    function getck(items, c, fn) {
      for (var i = 0, l = items.length; i < l; i++) {
        items[i].showcheck == true &&
          items[i].checkstate == 1 &&
          c.push(fn(items[i]));
    ...
    function getCkAndHalfCk(items, c, fn) {
      for (var i = 0, l = items.length; i < l; i++) {
        items[i].showcheck == true &&
          (items[i].checkstate == 1 || items[i].checkstate == 2) &&
          c.push(fn(items[i]));
        if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
  ...
  //扩展一下选中的方法
  $.fn.doCheckedNodes = function() {
    console.log(this[0].t);
  };
})(jQuery);
